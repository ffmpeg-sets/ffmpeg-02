...
#define STM_STRINGIZE(x) #x
#define STM_STRINGIZE2(x) STM_STRINGIZE(x)
#define STM_SHADER_STRING(text) @ STM_STRINGIZE2(text)

NSString *const vertexShaderString = STM_SHADER_STRING
(
...
NSString *const rgbFragmentShaderString = STM_SHADER_STRING
(
...
NSString *const yuvFragmentShaderString = STM_SHADER_STRING
(
 varying highp vec2 v_texcoord;
...
@protocol STMVideoGLRenderer
- (BOOL)isValid;
- (NSString *)fragmentShader;
- (void)resolveUniforms:(GLuint)program;
- (void)setFrame:(STMVideoFrame *)frame;
- (BOOL)prepareRender;
@end

@interface STMVideoGLRenderer_RGB : NSObject<STMVideoGLRenderer>
{
...
@implementation STMVideoGLRenderer_RGB
...
- (void)setFrame:(STMVideoFrame *)frame {
    STMVideoFrameRGB *rgbFrame = (STMVideoFrameRGB *)frame;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, frame.width, frame.height, 0, GL_BGRA, GL_UNSIGNED_BYTE, rgbFrame.rgb);
...
@interface STMVideoGLRenderer_YUV : NSObject<STMVideoGLRenderer>
{
...
@implementation STMVideoGLRenderer_YUV
...
- (void)setFrame:(STMVideoFrame *)frame {
    STMVideoFrameYUV *yuvFrame = (STMVideoFrameYUV *)frame;
...
@implementation STMVideoFrame

@end

@implementation STMVideoFrameRGB

@end

@implementation STMVideoFrameYUV
...
@implementation STMOpenGLView {
    ...
    id<STMVideoGLRenderer> _renderer;
}
...
- (id)initWithFrame:(CGRect)frame videoFrameSize:(CGSize)videoFrameSize videoFrameFormat:(STMVideoFrameFormat)videoFrameFormat {
    ...
    if (self) {
        ...
        if (videoFrameFormat == STMVideoFrameFormatYUV) {
            
            _renderer = [[STMVideoGLRenderer_YUV alloc] init];
            NSLog(@"OK use YUV GL renderer");
            
        } else {
            
            _renderer = [[STMVideoGLRenderer_RGB alloc] init];
...
- (void)updateVertices {
    ...
    const float width   = _videoFrameSize.width;
    const float height  = _videoFrameSize.height;
